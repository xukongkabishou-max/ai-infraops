...
function portalLink(requestId: number) {
  const configured = process.env.NEXT_PUBLIC_APPROVAL_PORTAL_URL?.trim();
  const base = new URL(configured || window.location.origin);
  if (!configured && base.port === "3001") base.port = "3000";
  return new URL(`/approvals/${requestId}`, base.origin).href;
}
...
export function ApprovalLink({ requestId }: { requestId: number }) {
  ...
  async function copy() {
    const link = portalLink(requestId);
    try {
  ...
  return <div className="flex min-w-0 flex-wrap items-center gap-3 text-xs">
    <button type="button" className="text-[#9fb0ff] underline underline-offset-4" onClick={() => window.location.assign(portalLink(requestId))}>审批详情</button>
